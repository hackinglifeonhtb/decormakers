        ...
        axios.post(`http://${process.env.REACT_APP_SERVER_LINK}/api/verify`, {token: localStorage.getItem('token') || ''}, {headers:{'x-access-token':localStorage.getItem('token'), 'email':localStorage.getItem('email')}})
            .then((res)=>{
                if(res.data.firstName !== undefined && res.data.secondName !== undefined)
                    ...
                    axios.post(`http://${process.env.REACT_APP_SERVER_LINK}/getCourse/${course_id}`, {email:localStorage.getItem('email')})
                        .then((course)=>{
                            ...
                            if(!course.data.courseAndTeacherData.progress.Lessons[lesson_name]) {
                                setTimeout(()=>{
                                    axios.post(`http://${process.env.REACT_APP_SERVER_LINK}/lessonCompleted/${course_id}/${lesson_name}`, {email:localStorage.getItem('email')})
                                        .then((res)=>{
                        ...
                        }}>
                            {
                                Object.keys(course[0]).length>0?
                                    course[0].lessons.map((lesson, index)=>{
                                        return (
                                            <Link to={`http://${process.env.REACT_APP_SITE_LINK}/courses/${course_id}/${lesson.Name}`} style={{textDecoration:'none'}}>
                                                <div style={{
